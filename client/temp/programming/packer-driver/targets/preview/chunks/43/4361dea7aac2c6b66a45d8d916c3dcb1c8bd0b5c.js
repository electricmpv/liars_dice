      ...
      _export("GameUI", GameUI = (_dec = ccclass('GameUI'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(_crd && PlayerDisplayController === void 0 ? (_reportPossibleCrUseOfPlayerDisplayController({
        ...
        handleStateChange(state) {
          ...
          if (this.playerDisplayController && state.players) {
            ...
            var playersDataArray = Object.entries(state.players).map(_ref => {
              var [sessionId, pState] = _ref;
              return {
                id: sessionId,
            ...
            var activePlayerIdsArray = state.activePlayerIds;
            var currentPlayerId = activePlayerIdsArray[state.currentPlayerIndex]; // Pass the converted PlayerData[]
          ...
          if (this.bidController) {
            ...
            if (state.status === 'playing') {
              ...
              var bidForController = this._currentBid[1] === 0 ? [0, 0] : [this._currentBid[0], this._currentBid[1]];
          ...
          if (state.status === 'finished' && !((_this$gameResultPanel = this.gameResultPanel) != null && _this$gameResultPanel.node.active) && state.players) {
            ...
            for (var p of Object.values(state.players)) {
        ...
        handleGameFinished(data) {
          ...
          if (this.gameResultPanel) {
            ...
            var finalState = (_crd && NetworkManager === void 0 ? (_reportPossibleCrUseOfNetworkManager({
              error: Error()
            ...
            if (finalState && finalState.players) {
              try {
                ...
                try {
                  Object.entries(finalState.players).forEach(_ref2 => {
                    var [sessionId, pState] = _ref2;
                    playersDataForResult.push({
                      id: sessionId,
                      name: pState.name,
                      isReady: pState.isReady,
          ...
          if (this.gameStatusLabel) {
            var winnerDisplayName = data.winnerName || (data.winnerSessionId ? "\u73A9\u5BB6 " + data.winnerSessionId.substring(0, 4) : "无");
        ...
        updateGameStatusLabel(state) {
          ...
          var currentPlayerId = state.activePlayerIds[state.currentPlayerIndex];
          var isMyTurn = currentPlayerId === this._sessionId;
          ...
          if (state.status === 'playing') {
            if (isMyTurn) {
              ...
              var currentPlayerName = ((_this$playerDisplayCo = this.playerDisplayController) == null ? void 0 : _this$playerDisplayCo.getPlayerNameWithAlias(currentPlayerId)) || "\u73A9\u5BB6 " + ((_currentPlayerId$subs = currentPlayerId == null ? void 0 : currentPlayerId.substring(0, 4)) != null ? _currentPlayerId$subs : '??');
            ...
            var winner = undefined; // Use client interface
            ...
            var winnerDisplayName = "无";
            var winnerSessionId = (_winner2 = winner) == null ? void 0 : _winner2.sessionId; // PlayerStateClient has sessionId
