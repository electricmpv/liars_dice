  ...
  return {
    ...
    execute: function () {
      ...
      _export("DiceItem", DiceItem = (_dec = ccclass('DiceItem'), _dec2 = property(Sprite), _dec3 = property(Label), _dec4 = property(Node), _dec5 = property(Animation), _dec6 = property([SpriteFrame]), _dec7 = property(Sprite), _dec(_class = (_class2 = class DiceItem extends Component {
        ...
        playRollAnimation(duration, callback) {
          if (duration === void 0) {
            duration = 1.0;
          }
          ...
          var originalSelected = this._isSelected; // 重置选中状态
          ...
          var rollSequence = () => {
            var shakeOffset = 8;
            var shakeTime = 0.05;
            var shakeCount = Math.floor(duration / shakeTime / 2);
            var shakeTween = tween(this.node); // 添加多次摇动

            for (var i = 0; i < shakeCount; i++) {
              // 随机方向摇动
              var xOffset = (Math.random() - 0.5) * 2 * shakeOffset;
              var yOffset = (Math.random() - 0.5) * 2 * shakeOffset;
        ...
        rollToValue(finalValue, duration, callback) {
          if (duration === void 0) {
            duration = 1.0;
          }
