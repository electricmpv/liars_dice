    ...
    execute: function () {
      ...
      _export("GameStateManager", GameStateManager = class GameStateManager extends EventTarget {
        ...
        handleChallengeResult(data) {
          ...
          var challenger = this._gameState.players.find(p => p.id === challengerId);

          var challenged = this._gameState.players.find(p => p.id === challengedId);
          ...
          var challengeSucceeded = !isValid; // If bid was NOT valid, challenge succeeded
        ...
        addHistoryItem(item) {
          // 检查是否已存在相同ID的历史记录
          var existingIndex = this._historyItems.findIndex(h => h.id === item.id);
        ...
        addBidHistory(playerId, bid, playerName) {
          ...
          var [value, count] = bid; // Destructuring should be safe now

          var faceLabel = this.getFaceLabel(value);
          this.addHistoryItem({
        ...
        get isPlayerTurn() {
          ...
          var isMyTurn = this._gameState.activePlayers && this._gameState.currentPlayerIndex !== undefined && this._gameState.activePlayers[this._gameState.currentPlayerIndex] === this._currentPlayerId;
