    ...
    execute: function () {
      ...
      _export("BidController", BidController = (_dec = ccclass('BidController'), _dec2 = property(Button), _dec3 = property(Button), _dec4 = property(Button), _dec5 = property(Node), _dec6 = property(ScrollView), _dec7 = property(Node), _dec8 = property(Prefab), _dec9 = property(Label), _dec(_class = (_class2 = class BidController extends Component {
        ...
        populateInitialCountItems() {
          ...
          for (var i = 1; i <= this._totalDiceInGame; i++) {
            var itemNode = instantiate(this.countListItemPrefab);
            var countListItem = itemNode.getComponent(_crd && CountListItem === void 0 ? (_reportPossibleCrUseOfCountListItem({
              error: Error()
        ...
        updateState(isMyTurn, currentBid, totalDiceInGame) {
          ...
          var hasPreviousBid = this._currentBid[1] > 0;
          ...
          var [currentValueFace, currentCount] = this._currentBid;

          if (currentCount === 0) {
            this._minSelectableCount = 1;
          } else {
            // If current bid is max possible value (1, effective value 7), must increase count
            var currentEffectiveValue = (_crd && BidValidator === void 0 ? (_reportPossibleCrUseOfBidValidator({
              error: Error()
        ...
        updateCurrentBidDisplay() {
          ...
          var [value, count] = this._currentBid;
        ...
        handleCountSelection(detail) {
          ...
          var minFace = (_crd && BidValidator === void 0 ? (_reportPossibleCrUseOfBidValidator({
            error: Error()
        ...
        onValueSelect(event, valueString) {
          ...
          var parsedValue = parseInt(valueString);
        ...
        _updateFaceButtons(minFace) {
          ...
          this.valueSelectorNode.children.forEach(btnNode => {
            var button = btnNode.getComponent(Button); // Assuming button nodes are named like "ValueButton1", "ValueButton2", etc.

            var faceValue = parseInt(btnNode.name.replace('ValueButton', ''));
        ...
        _updateAvailableQuantities() {
          ...
          this._instantiatedCountItems.forEach((item, index) => {
            var isSelectable = item.countValue >= this._minSelectableCount; // Node is active (visible) if count is >= minimum required
        ...
        scrollToFirstAvailableQuantity(firstVisibleIndex) {
          ...
          if (firstVisibleIndex >= 0 && this._instantiatedCountItems.length > 0) {
            ...
            var percentage = firstVisibleIndex / this._instantiatedCountItems.length;
        ...
        onBidClick() {
          ...
          var bid = [this._selectedBidValue, this._selectedBidCount];
        ...
        validateAndEnableBidButton() {
          ...
          var isValid = false;
          var reason = []; // Array to store reasons for validation result
          ...
          if (this._isMyTurn && this._selectedBidCount > 0 && (_crd && BidValidator === void 0 ? (_reportPossibleCrUseOfBidValidator({
            ...
            var bidCheckResult = (_crd && BidValidator === void 0 ? (_reportPossibleCrUseOfBidValidator({
              error: Error()
            }), BidValidator) : BidValidator).isBidValid(this._selectedBidValue, this._selectedBidCount, this._currentBid, this._totalDiceInGame);
        ...
        highlightSelectedCountItem(selectedNode) {
          ...
          if (this._selectedCountListItemNode && this._selectedCountListItemNode.isValid) {
            var label = this._selectedCountListItemNode.getComponentInChildren(Label);

            var button = this._selectedCountListItemNode.getComponent(Button);
          ...
          if (this._selectedCountListItemNode && this._selectedCountListItemNode.isValid) {
            var _label = this._selectedCountListItemNode.getComponentInChildren(Label);

            if (_label) {
              _label.color = Color.YELLOW; // Highlight color
            }
