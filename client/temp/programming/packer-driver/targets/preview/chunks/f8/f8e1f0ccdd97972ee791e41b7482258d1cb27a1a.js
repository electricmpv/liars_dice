  ...
  return {
    ...
    execute: function () {
      ...
      Player = class Player {
        ...
        buildHand() {
          ...
          return _asyncToGenerator(function* () {
            _this.hand = yield GameEngine.rollDices(_this.diceLeft);
          })();
        }
      ...
      GameBid = class GameBid {
        constructor(value, count, playerId) {
          if (value === void 0) {
            value = 0;
          }
      ...
      _export("default", GameEngine = class GameEngine {
         ...
         */
        constructor(gameId, playerNames, initialDiceCount) {
          if (initialDiceCount === void 0) {
            initialDiceCount = 5;
          }
          ...
          for (var i = 0; i < playerNames.length; i++) {
            var player = new Player(i + 1, playerNames[i], initialDiceCount);
        ...
        getCurrentPlayer() {
          var playerIndex = this.activePlayers[this.currentPlayerIndex];
        ...
        checkBidValidity(bid) {
          var count = 0;
          var value = bid.value; // 统计所有玩家手中符合条件的骰子数量

          for (var playerIndex of this.activePlayers) {
            var player = this.players[playerIndex];
            var hand = player.hand || [];

            for (var dice of hand) {
              if (dice === value) {
                count++;
              }
            }
        ...
        startNewRound() {
          ...
          return _asyncToGenerator(function* () {
            _this2.roundNumber++;
            _this2.moveNumber = 0;
            _this2.currentBid = new GameBid(); // 为所有玩家生成新的骰子

            var handPromises = _this2.activePlayers.map(playerIndex => _this2.players[playerIndex].buildHand());

            yield Promise.all(handPromises);
            console.log("\u56DE\u5408 " + _this2.roundNumber + " \u5F00\u59CB!");
          })();
        }
        /**
         * 处理玩家竞价
        ...
        challengeBid(playerId) {
          ...
          var previousBidValid = this.checkBidValidity(this.currentBid); // 触发质疑事件
          ...
          for (var i = 0; i < this.players.length; i++) {
            if (this.players[i].id === playerId) {
              ...
              if (this.players[i].diceLeft <= 0) {
                var playerIndexInActive = this.activePlayers.indexOf(i);
                ...
                if (this.activePlayers.length === 1) {
                  var winnerId = this.players[this.activePlayers[0]].id;
