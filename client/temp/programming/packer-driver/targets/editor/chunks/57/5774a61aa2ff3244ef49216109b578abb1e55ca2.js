    ...
    execute: function () {
      ...
      _export("BidValidator", BidValidator = class BidValidator {
        ...
        static isBidValid(value, count, currentBid, totalDice) {
          const [currentValueFace, currentCount] = currentBid; // 基本验证: 点数必须是 1-6, 数量必须大于 0 且不大于总骰子数
          ...
          const newValue = this.getEffectiveValue(value); // 新规则: 如果数量增加，叫价总是有效的
        ...
        static getMinValidCount(selectedValue, currentBid) {
          const [currentValueFace, currentCount] = currentBid; // 首次叫价，最小数量为 1
          ...
          const currentValue = this.getEffectiveValue(currentValueFace);
          const newValue = this.getEffectiveValue(selectedValue); // 新规则: 如果选定点数的有效值 > 当前点数的有效值，可以保持相同数量
        ...
        static getMinValidFace(selectedCount, currentBid) {
          const [currentValueFace, currentCount] = currentBid; // 首次叫价，最小点数是 1
          ...
          if (selectedCount === currentCount) {
            const currentValue = this.getEffectiveValue(currentValueFace); // 从 1 开始查找第一个有效值大于 currentValue 的点数
