      ...
      _export("PlayerDisplayController", PlayerDisplayController = (_dec = ccclass('PlayerDisplayController'), _dec2 = property(Node), _dec3 = property(Prefab), _dec(_class = (_class2 = class PlayerDisplayController extends Component {
        ...
        updatePlayerDisplays(players, currentPlayerId) {
          ...
          players.forEach(player => {
            let playerNode = oldNodes.get(player.id); // 如果节点不存在，则创建新的
        ...
        updateSinglePlayerDisplay(playerNode, playerData, currentPlayerId) {
          ...
          const nameLabel = (_playerNode$getChildB = playerNode.getChildByName('NameLabel')) == null ? void 0 : _playerNode$getChildB.getComponent(Label); // 修正查找路径：PlayerInfo -> DiceInfo -> DiceCountLabel

          const diceInfoNode = playerNode.getChildByName('DiceInfo');
          const diceCountLabel = diceInfoNode == null || (_diceInfoNode$getChil = diceInfoNode.getChildByName('DiceCountLabel')) == null ? void 0 : _diceInfoNode$getChil.getComponent(Label); // const background = playerNode.getChildByName('Background'); // 用于高亮
          // 更新名字和颜色
        ...
        getPlayerNameWithAlias(playerId) {
          ...
          if (playerNode && playerNode.isValid) {
            ...
            const nameLabel = (_playerNode$getChildB2 = playerNode.getChildByName('NameLabel')) == null ? void 0 : _playerNode$getChildB2.getComponent(Label);
