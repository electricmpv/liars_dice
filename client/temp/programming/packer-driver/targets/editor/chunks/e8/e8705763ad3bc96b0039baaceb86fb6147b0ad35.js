    ...
    execute: function () {
      ...
      _export("LobbyController", LobbyController = (_dec = ccclass('LobbyController'), _dec2 = property(Prefab), _dec3 = property(Prefab), _dec4 = property(Prefab), _dec5 = property(Prefab), _dec6 = property(Prefab), _dec7 = property(Prefab), _dec8 = property(Node), _dec9 = property(Node), _dec10 = property(Node), _dec11 = property(Node), _dec12 = property(Node), _dec13 = property(Node), _dec14 = property(ScrollView), _dec15 = property(ScrollView), _dec16 = property(ScrollView), _dec17 = property(Node), _dec18 = property(Node), _dec19 = property(Node), _dec20 = property(Node), _dec21 = property(Node), _dec22 = property(Node), _dec23 = property(Node), _dec(_class = (_class2 = class LobbyController extends Component {
        ...
        async loadInitialData() {
          try {
            // 显示加载中通知
            this.showNotice("正在加载数据...", "info"); // 并行加载数据

            await Promise.all([this._roomService.fetchRooms(), this._friendService.fetchFriends(), this._chatService.connect()]); // 显示成功通知
        ...
        createTabGroup(groupId, tabs, callback) {
          ...
          tabs.forEach((tab, index) => {
            ...
            const tabComp = tabNode.getComponent(_crd && FilterTab === void 0 ? (_reportPossibleCrUseOfFilterTab({
              error: Error()
            }), FilterTab) : FilterTab);
        ...
        updateFilterTabsVisibility() {
          ...
          this._filterTabs.forEach((tabs, groupId) => {
            // 根据当前状态决定是否显示
            const visible = groupId === 'room' && this._currentState === LobbyState.ROOM_LIST || groupId === 'friend' && this._currentState === LobbyState.FRIEND_LIST; // 设置标签显示/隐藏
        ...
        updateRoomList(rooms) {
          ...
          const filteredRooms = this._roomService.getFilteredRooms(rooms); // 添加房间项
          ...
          filteredRooms.forEach(roomAvailable => {
            ...
            const adaptedInfo = (_crd && RoomService === void 0 ? (_reportPossibleCrUseOfRoomService({
              error: Error()
            ...
            const roomComp = roomNode.getComponent(_crd && RoomItem === void 0 ? (_reportPossibleCrUseOfRoomItem({
              error: Error()
        ...
        updateFriendList(friends) {
          ...
          const filteredFriends = this._friendService.getFilteredFriends(friends); // 添加好友项
        ...
        addChatMessage(message) {
          ...
          const chatComp = chatNode.getComponent(_crd && ChatItem === void 0 ? (_reportPossibleCrUseOfChatItem({
            error: Error()
          }), ChatItem) : ChatItem);
        ...
        showNotice(text, type = 'info', duration, isPersistent = false) {
        ...
        showFriendRequest(requestInfo) {
          ...
          const requestComp = requestNode.getComponent(_crd && FriendRequest === void 0 ? (_reportPossibleCrUseOfFriendRequest({
            error: Error()
